...
(function($) {
	...
	// Homepage Jovian hero.
		(function() {
			...
			function setupHeroScrollVideo(heroElement) {
				var video = heroElement.querySelector('.hero-video'),
					videoLayer,
					prefersReducedMotion = window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches,
					introSeconds = 0.35,
					scrubStartSeconds = 0.32,
					scrubEndPaddingSeconds = 0.16,
					scrollCompleteAt = 0.94,
					duration = 0,
					introTimer = null,
					introActive = true,
					targetProgress = 0,
				...
				videoLayer = video.parentElement || heroElement;
				video.loop = false;
				...
				function scrollProgress() {
					var travel = Math.max(heroElement.offsetHeight * 0.98, window.innerHeight * 0.86, 1);

					return clamp(window.scrollY / (travel * scrollCompleteAt), 0, 1);
				}
				...
				function applyProgress(progress, timestamp) {
					var shouldFinalize = Math.abs(targetProgress - renderedProgress) < 0.002,
						eased = 1 - Math.pow(1 - progress, 2),
						x = -24 * eased,
						y = 36 * eased,
						scale = 0.84 + 0.035 * eased,
						scrubEndSeconds,
						targetTime;

					videoLayer.style.setProperty('--hero-video-x', x.toFixed(1) + 'px');
					videoLayer.style.setProperty('--hero-video-y', y.toFixed(1) + 'px');
					videoLayer.style.setProperty('--hero-video-scale', scale.toFixed(3));

					if (prefersReducedMotion || !syncDuration())
						return;

					scrubEndSeconds = Math.max(scrubStartSeconds, duration - scrubEndPaddingSeconds);
					targetTime = scrubStartSeconds + progress * (scrubEndSeconds - scrubStartSeconds);

					if (
						Number.isFinite(targetTime) &&
						Math.abs(video.currentTime - targetTime) > 0.012 &&
						(shouldFinalize || timestamp - lastVideoUpdate > 30)
					) {
						try {
				...
				function animateScrollState(timestamp) {
					var delta,
						step;

					animationFrame = null;
					targetProgress = scrollProgress();
					delta = targetProgress - renderedProgress;

					if (Math.abs(delta) < 0.002)
						renderedProgress = targetProgress;
					else {
						step = clamp(delta * 0.14, -0.018, 0.018);
						renderedProgress += step;
					}

					applyProgress(renderedProgress, timestamp || 0);
				...
				function requestScrollState() {
					...
					animationFrame = window.requestAnimationFrame(animateScrollState);
				}
				...
				function startIntro() {
					...
					introTimer = window.setTimeout(finishIntro, introSeconds * 1000);
					requestScrollState();
				}
			...
			setupHeroScrollVideo(hero);

			if (!svg || !jupiterLinesGroup)
				return;
